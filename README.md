...
> **Note (cron + ionice/nice):** If you use a more aggressive line for the LXC backup sync like:
>
> ```bash
...
> ```
>
> and the script does **not** run, the most common cause is that `cron` cannot find the binaries `ionice` and/or `nice` because of a restricted `PATH`.
>
> A professional and robust fix is to define a full PATH at the top of the root crontab:
>
> ```bash
> PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
> ```>
> This ensures `cron` can locate `ionice`, `nice`, and other system commands, so `/root/sync_lxc_backups.sh` runs reliably even when wrapped with `ionice`/`nice`.
...
> All detailed version history is maintained in [`CHANGELOG.md`](CHANGELOG.md).

### 🏷️ Versioning Scheme

Versions follow the format **MAJOR.MINOR.PATCH**:

- **MAJOR** — Breaking changes or incompatible architecture updates  
- **MINOR** — Backwards-compatible new features  
- **PATCH** — Bug fixes and small enhancements  

### 📦 Release Template (for new versions)

When you cut a new release, add an entry in `CHANGELOG.md` using this structure:
...
For the detailed history of `ReProxm v1.0 ("Resilience")`, see the dedicated entry in [`CHANGELOG.md`](CHANGELOG.md).
